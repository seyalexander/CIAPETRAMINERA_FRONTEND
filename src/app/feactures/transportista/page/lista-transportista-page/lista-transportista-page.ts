...
import { TransportistaService } from '../../service/api/transportista-service';
...
@Component({
  selector: 'app-lista-transportista-page',
  imports: [
    ButtonModule,
    IconFieldModule,
    InputIconModule,
  ...
  templateUrl: './lista-transportista-page.html',
  styleUrl: './lista-transportista-page.css',
  providers: [ConfirmationService, MessageService],
})
export class ListaTransportistaPage {

  items: MenuItem[] | undefined;

  transportistas: TransportistaModel[] = [];
  detalleTransportistas: TransportistaModel = {} as TransportistaModel;
  ...
  ObtenerListaClientes(estado: number): void {
    this.loading = true;
    const request: RequestListaTransportistas = { estado };
    this.transportistaService.ListarTransportistas(request).subscribe({
      next: (resp: ResponseListaTransportista) => {
        if (resp.exito) {
  ...
  ConfirmacionAnular(event: Event, transportista: TransportistaModel) {
    this.confirmationService.confirm({
      target: event.currentTarget as EventTarget,
      message: 'Está seguro de desactivar este transportista?',
  ...
  ConfirmacionActivar(event: Event, transportista: TransportistaModel) {
    this.confirmationService.confirm({
      target: event.currentTarget as EventTarget,
  ...
  showDialogDetalle(detalle: TransportistaModel) {
    this.detalleTransportistas = detalle;
    this.visibleDetalle = true;
    this.cdr.detectChanges();
  }

  showDialogEditar(detalle: TransportistaModel) {
    this.detalleTransportistas = detalle;
